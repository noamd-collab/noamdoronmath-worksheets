/**
 * M27: live SOURCE → served PREVIEW parity for each of the 15 batch posts
 * (+ optional all served). Compares content blocks (h1/h2/h3/para skim) and
 * every visible same-site link destination policy.
 *
 * Usage: PREVIEW_BASE=https://… node scripts/check-source-served-blog-m27.mjs
 */
...
const posts = readdirSync('src/data/blog-posts')
  ...
  .filter((p) => p.batch === 'm27');

mkdirSync('reports/m27-blog', { recursive: true });
...
await browser.close();
writeFileSync(
  'reports/m27-blog/source-served-parity.json',
...
console.log('\nAll M27 source→served parity checks passed');
