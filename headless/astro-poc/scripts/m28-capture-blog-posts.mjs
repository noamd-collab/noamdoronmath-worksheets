/**
 * M28: capture 18 additional blog posts using the M25 whole-page extractor + chrome.
 * Reads picks from reports/m28-blog/batch-candidates.json (stable fileSlug per pick).
 */
...
const picks = JSON.parse(readFileSync('reports/m28-blog/batch-candidates.json', 'utf8')).picks;
mkdirSync('src/data/blog-posts', { recursive: true });
mkdirSync('reports/m28-blog/captures', { recursive: true });
...
for (const pick of picks) {
  ...
  writeFileSync(
    `reports/m28-blog/captures/${ids.fileSlug}.json`,
...
writeFileSync(
  'reports/m28-blog/batch-manifest.txt',
  `# HEADLESS-MIGRATION-27 blog batch (15 of remaining 51)
# Reuses M25 whole-page capture + chrome extractor.
# Categories: middle-school, elementary, teachers-and-parents, general guides.
# Layout clusters: related+medium, medium, long-body, lists+long, many-links.
...
## Deferred after M28
- remaining 36 /post/* bodies
`
);

writeFileSync(
  'reports/m28-blog/batch-results.json',
  JSON.stringify({ generatedAt: new Date().toISOString(), results }, null, 2) + '\n'
);
console.log('\nWrote M28 batch manifest +', results.length, 'JSON files');
