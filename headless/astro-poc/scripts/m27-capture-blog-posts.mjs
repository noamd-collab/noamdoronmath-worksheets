/**
 * M27: capture 15 additional blog posts using the M25 whole-page extractor + chrome.
 * Reads picks from reports/m27-blog/batch-candidates.json (stable fileSlug per pick).
 */
...
const picks = JSON.parse(readFileSync('reports/m27-blog/batch-candidates.json', 'utf8')).picks;
mkdirSync('src/data/blog-posts', { recursive: true });
mkdirSync('reports/m27-blog/captures', { recursive: true });
...
for (const pick of picks) {
  ...
  writeFileSync(
    `reports/m27-blog/captures/${ids.fileSlug}.json`,
...
writeFileSync(
  'reports/m27-blog/batch-manifest.txt',
...
writeFileSync(
  'reports/m27-blog/batch-results.json',
  JSON.stringify({ generatedAt: new Date().toISOString(), results }, null, 2) + '\n'
);
console.log('\nWrote M27 batch manifest +', results.length, 'JSON files');
