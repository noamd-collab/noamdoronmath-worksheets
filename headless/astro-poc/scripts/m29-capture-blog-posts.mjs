/**
 * M29: capture remaining 18 blog posts using the M25 whole-page extractor + chrome.
 * Reads picks from reports/m29-blog/batch-candidates.json (stable fileSlug per pick).
 */
...
const picks = JSON.parse(readFileSync('reports/m29-blog/batch-candidates.json', 'utf8')).picks;
mkdirSync('src/data/blog-posts', { recursive: true });
mkdirSync('reports/m29-blog/captures', { recursive: true });
...
for (const pick of picks) {
  ...
  writeFileSync(
    `reports/m29-blog/captures/${ids.fileSlug}.json`,
...
writeFileSync(
  'reports/m29-blog/batch-manifest.txt',
...
writeFileSync(
  'reports/m29-blog/batch-results.json',
  JSON.stringify({ generatedAt: new Date().toISOString(), results }, null, 2) + '\n'
);
console.log('\nWrote M29 batch manifest +', results.length, 'JSON files');
