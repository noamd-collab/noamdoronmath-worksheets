(function(){
 ...
 var cfg=window.NOAM_LEARNING_CONFIG||{},portal=new URL('learning.html',location.href).href;
 // OAuth redirect must be same-origin (or exact fixed origin) so PKCE verifier stays on this origin.
 // GitHub Pages callback alone does NOT establish a Headless session.
 var AuthRedirect=window.NoamLearningAuthRedirect;
 ...
 async function signInWithGoogle(){
  ...
  try{
   var resolved=AuthRedirect&&AuthRedirect.resolveAuthRedirectTo
    ?AuthRedirect.resolveAuthRedirectTo(cfg,location.href)
    :{ok:true,redirectTo:new URL('learning.html',location.href).href};
   if(!resolved.ok){throw new Error(resolved.error||'AUTH_REDIRECT');}
   var r=await client.auth.signInWithOAuth({provider:'google',options:{redirectTo:resolved.redirectTo,queryParams:{prompt:'select_account'}}});
   if(r.error)throw r.error;
  }
  catch(e){signInPending=false;renderAuth();tell('לא הצלחנו לפתוח את הכניסה בגוגל. אפשר להמשיך ללא כניסה.');}
 }
 function openPdf(item){
  var raw=item.pdf,full=/^https:\/\//.test(raw)?raw:'https://static.wixstatic.com/ugd/d8e7ad_'+raw+'.pdf';
