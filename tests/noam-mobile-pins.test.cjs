...
function fixture(exercises, narrow = true, pageHeight = 500) {
  const pages = new Map();
  ...
  const media = { matches: narrow, addEventListener(name, callback) { assert.equal(name, "change"); this.onChange = callback; } };
  ...
  return { ctx, media, pages, picker, selections, pins: () => [...pages.values()].flatMap(page => page.children) };
}
