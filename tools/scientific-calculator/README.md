...
Independent educational browser implementation for Noam Doron Math. The physical reference is the **original silver fx-991ES PLUS**, matching the three scans supplied by the site owner on 2026-09-09. Its ivory number keys, orange DEL/AC keys and circular REPLAY replace the earlier black second-edition skin. No Casio firmware, ROM, photograph or proprietary software is included. Casio branding in the physical reference does not imply affiliation.
...
- `make-ui.cjs` and `device.css` define the physical shell. `natural-editor.js` owns the structured input slots, token cursor, capture and serialization; `device.js` handles the keys, LCD and settings. `modes.js` handles advanced mode workflows. `core.js`, `algorithms.js` and `catalog.js` implement the calculations.
...
- Browser MathML and fonts approximate the hardware's dot-matrix display. LCD layout adapts to the browser rather than reproducing every screen pixel and firmware state. The photographed fraction boundaries and explicit manual sequences are tested; other structural choices are identified as inferred in the navigation audit.
...
- [Original silver PLUS user guide](https://support.casio.com/pdf/004/fx-570_991ES_PLUS_E.pdf)
- [PLUS second-edition guide, used for the earlier numerical implementation](https://support.casio.com/global/en/calc/manual/fx-570ESPLUS_991ESPLUS_en/)
...
## September 2026 training refinement

Natural input no longer inserts a fake cursor into the evaluation string and reparses it. Fractions, roots, powers, groups and function arguments are structured fields. RIGHT exits a completed denominator at its parent baseline; UP/DOWN visits populated siblings; explicit parentheses are preserved while generated evaluation parentheses stay hidden. The result is hidden while entering a new expression.

The regression suite uses the actual built HTML and physical key clicks, including the owner's 3/2 example, the manual fraction/root examples, Natural INS argument capture, history/replay editing and direct MthIO selection for the original silver model. See `src/training-sequences.test.cjs` and `src/natural-editor.test.cjs`. The source audit distinguishes photographs, explicit manual evidence and inferred navigation rules; it is retained in the local handoff output.
